...
function upwind_scheme!(compo_f, compo_f_prev, v_c, Δt, grid, parameters)
    ...
    @inbounds @threads for I = CartesianIndices(compo_f)
        ...
        if v_c[:x][i,j] > 0
            if v_c[:z][i,j] > 0
                compo_f[I] = compo_f_prev[I] - (Δt / Δz_ad) * v_c[:z][i,j] * (compo_f_prev[I] - compo_f_prev[is,j,k]) - (Δt / Δx_ad) * v_c[:x][i,j] * (compo_f_prev[I] - compo_f_prev[i,jw,k])
            else v_c[:z][i,j] < 0
                compo_f[I] = compo_f_prev[I] - (Δt / Δz_ad) * v_c[:z][i,j] * (compo_f_prev[in,j,k] - compo_f_prev[I]) - (Δt / Δx_ad) * v_c[:x][i,j] * (compo_f_prev[I] - compo_f_prev[i,jw,k])
            end
        else
            if v_c[:z][i,j] > 0
                compo_f[I] = compo_f_prev[I] - (Δt / Δz_ad) * v_c[:z][i,j] * (compo_f_prev[I] - compo_f_prev[is,j,k]) - (Δt / Δx_ad) * v_c[:x][i,j] * (compo_f_prev[i,je,k] - compo_f_prev[I])
            else v_c[:z][i,j] < 0
                compo_f[I] = compo_f_prev[I] - (Δt / Δz_ad) * v_c[:z][i,j] * (compo_f_prev[in,j,k] - compo_f_prev[I]) - (Δt / Δx_ad) * v_c[:x][i,j] * (compo_f_prev[i,je,k] - compo_f_prev[I])
            end
        end
