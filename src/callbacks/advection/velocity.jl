...
#     @inbounds @threads for I in CartesianIndices(q_f[:z])
#         i,j = Tuple(I)
...
function fluid_flux_ad!(qc_f, ϕ, Pe, vc_s, Properties, Domain, Grid)
    ...
    @inbounds @threads for I in CartesianIndices(qc_f[:z])
        ...
        qc_f[:z][I] = ϕ[I]^n * ((avzPe(i,in,j) - avzPe(is,i,j)) / Δz_ad + (ρ_s - ρ_f[i,j]) / (ρ0)) #+ vc_s[:z][I]

        qc_f[:x][I] = ϕ[I]^n * (avxPe(i,j,je) - avxPe(i,jw,j)) / Δx_ad #+ vc_s[:x][I]
    end
...
#     @inbounds @threads for I in CartesianIndices(v_f[:z])
#         i,j = Tuple(I)
#         is, in = limit_periodic(i-1, nz), limit_periodic(i, nz)
...
#     @inbounds @threads for I in CartesianIndices(v_f[:x])
#         i,j = Tuple(I)
#         jw, je = limit_periodic(j-1, nx), limit_periodic(j, nx)
...
function solid_velocity_ad!(vc_s, ϕ, ϕ_prev, Δt, Properties, Domain, Grid)
    ...
    @inbounds @threads for I in CartesianIndices(vc_s[:z])
