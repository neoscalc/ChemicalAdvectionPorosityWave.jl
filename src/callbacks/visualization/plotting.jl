...
function plotting_tpf(u, t, integrator)
    ...
    @unpack compaction_ϕ, compaction_Pe, compaction_t, c0, v_f, ϕ_ini, vc_s, v_s, vc_f = integrator.p[:parameters]
    ...
    p4 = heatmap(x*1e-3, z*1e-3, vc_f[:z],  xlim=(0, last(x)*1e-3), ylim=(0, last(z)*1e-3), title="Fluid velocity \n(z component, adimensional)", xlabel="Distance (km)")
